class Solution:
    ...
    def binarySearchSpaceComIs1(self, nums: List[int], target: int, left: int, right: int) -> int:
        while left <= right:
            mid = left + (right - left) // 2
    ...
    def search(self, nums: List[int], target: int) -> int:
        ...
        # (1) using Recursion Stack 
        #  => Time Complexity O(log n), Space Complexity O(log n)

        # (2) Iterative version
        #  => Time Complexity O(log n), Space Complexity O(1)

        return self.binarySearchSpaceComIs1(nums, target, 0, len(nums) - 1)
