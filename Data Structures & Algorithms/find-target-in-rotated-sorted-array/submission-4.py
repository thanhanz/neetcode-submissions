class Solution:
    ...
    def binarySearchTimeComIs1(self, nums: List[int], target: int, left: int, right: int) -> int:
        while left <= right:
            mid = left + (right - left) // 2
    ...
    def search(self, nums: List[int], target: int) -> int:
        ...
        #This is O(log n) -> using Recursion Stack
        return self.binarySearchTimeComIs1(nums, target, 0, len(nums) - 1)
